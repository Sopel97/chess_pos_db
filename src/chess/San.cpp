...
namespace san
{
    ...
    namespace detail
    {
        ...
        [[nodiscard]] static FORCEINLINE constexpr bool contains(std::string_view s, char c)
        {
            return s.find(c) != std::string::npos;
        }

        [[nodiscard]] static FORCEINLINE constexpr bool isSanCapture(std::string_view san)
        {
        ...
        static FORCEINLINE inline std::size_t removeSanCapture(char* san, std::size_t length)
        {
        ...
        {
            static constexpr std::array<bool, 256> isDecoration = []() {
        ...
        static FORCEINLINE inline std::size_t removeSanDecorations(char* san, std::size_t length)
        {
        ...
        [[nodiscard]] static FORCEINLINE constexpr bool isPromotedPieceType(char c)
        {
            return c == 'N' || c == 'B' || c == 'R' || c == 'Q';
        }

        [[nodiscard]] static FORCEINLINE constexpr PieceType parsePromotedPieceType(char c)
        {
            switch (c)
            {
            case 'N':
        ...
        [[nodiscard]] static FORCEINLINE constexpr char pieceTypeToChar(PieceType pt)
        {
            switch (pt)
            {
        ...
        [[nodiscard]] static Move sanToMove_Pawn(const Position& pos, const char* san, std::size_t sanLen)
        {
        ...
        [[nodiscard]] static Move sanToMove(const Position& pos, const char* san, std::size_t sanLen)
        {
            static_assert(
        ...
        template Move sanToMove<PieceType::Knight>(const Position&, const char*, std::size_t);
        template Move sanToMove<PieceType::Bishop>(const Position&, const char*, std::size_t);
        template Move sanToMove<PieceType::Rook>(const Position&, const char*, std::size_t);
        template Move sanToMove<PieceType::Queen>(const Position&, const char*, std::size_t);

        [[nodiscard]] static Move sanToMove_King(const Position& pos, const char* san, std::size_t length)
        {
        ...
        [[nodiscard]] static Move sanToMove_Castle(const Position& pos, const char* san, std::size_t length)
        {
            // either:
            // 012345 - idx
            // O-O-O
        ...
        [[nodiscard]] static std::optional<Move> trySanToMove_Pawn(const Position& pos, const char* san, std::size_t sanLen)
        {
            // since we remove capture information it's either
            // 012345 idx
            // a1
        ...
        template <PieceType PieceTypeV>
        [[nodiscard]] static std::optional<Move> trySanToMove(const Position& pos, const char* san, std::size_t sanLen)
        {
            static_assert(
                PieceTypeV == PieceType::Knight
                || PieceTypeV == PieceType::Bishop
        ...
        template std::optional<Move> trySanToMove<PieceType::Bishop>(const Position&, const char*, std::size_t);
        template std::optional<Move> trySanToMove<PieceType::Rook>(const Position&, const char*, std::size_t);
        template std::optional<Move> trySanToMove<PieceType::Queen>(const Position&, const char*, std::size_t);

        [[nodiscard]] static std::optional<Move> trySanToMove_King(const Position& pos, const char* san, std::size_t length)
        {
            // since we remove captures the only possible case is 
            // Ka1
        ...
        [[nodiscard]] static std::optional<Move> trySanToMove_Castle(const Position& pos, const char* san, std::size_t length)
        {
            // either:
            // 012345 - idx
            // O-O-O
        ...
        {
            static constexpr std::array<Move(*)(const Position&, const char*, std::size_t), 256> funcs = []() {
        ...
        [[nodiscard]] static Move sanToMove(const Position& pos, char* san, std::size_t length)
        {
        ...
        [[nodiscard]] static std::optional<Move> trySanToMove(const Position& pos, char* san, std::size_t length)
        {
            // ?[NBRQK]?[a-h]?[1-8]?x[a-h][1-8]
            // *above regex contains all valid SAN strings
            // (but also some invalid ones)
        ...
        static void appendUnambiguousCompactFrom(const Position& pos, const Move& move, std::string& san)
        {
    ...
    template std::string moveToSan<SanSpec::None>(const Position&, Move);
    template std::string moveToSan<SanSpec::Full>(const Position&, Move);

    namespace detail
    {
        static constexpr std::array<bool, 256> validStart = []() {
    ...
    {
        return detail::validStart[static_cast<unsigned char>(c)];
    }
